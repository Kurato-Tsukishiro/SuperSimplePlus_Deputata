...
IN_FILE = os.path.join(WORKING_DIR, "TranslationData.xlsx")
OUT_FILE = os.path.join(WORKING_DIR, "SuperSimplePlus",
                        "Resources", "TranslationData.json")
      ...
      for row in rows:
        ...
        for i, string in enumerate(row[1:]):
          if string.value:
            ...
            data[i] = string.value.replace("\r", "").replace(
                "_x000D_", "").replace("\\n", "\n")
...
if __name__ == "__main__":
  in_files = [
      os.path.join(WORKING_DIR, "TranslationData.xlsx"),
  ]
